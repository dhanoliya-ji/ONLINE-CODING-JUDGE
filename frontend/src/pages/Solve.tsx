...
export function Solve() {
  ...
  // Restore any draft for this problem/language pair, else seed a starter.
  useEffect(() => {
    if (!slug) return;
    ...
    <div className="flex flex-col items-center gap-3 py-10">
      <div className="flex gap-1.5" aria-hidden="true">
        {[0, 1, 2].map((index) => (
          <span
            key={index}
            className="size-2.5 animate-bounce rounded-full bg-violet-400"
      ...
      <p className="text-sm text-violet-200/70">
        {mode === "submit"
          ? "Judging against every test case…"
          : "Running your code…"}
      </p>
      <p className="max-w-xs text-center text-xs text-violet-200/40">
        Code executes in a sandbox with hard time and memory limits.
      </p>
    </div>
  );
}
...
function SubmissionResult({ submission }: { submission: Submission }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Progress across the suite — instantly legible at a glance. */}
          ...
          <div className="space-y-2">
            {submission.test_results.map((test) => (
              ...
              >
                ...
                {/* Only sample cases carry a diff; hidden data never leaves
                    the server, so there is nothing to show for those. */}
...
function RunOutput({ result }: { result: RunResult }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Output label="stdout" body={result.stdout || "(no output)"} />
      {result.stderr && <Output label="stderr" body={result.stderr} tone="fail" />}
      {result.compile_output && (
        <Output label="compiler" body={result.compile_output} tone="fail" />
      )}
    </div>
  );
}
...
function Output({
  ...
  return (
    <div>
      <p className="mb-1.5 text-xs font-semibold tracking-wide text-violet-200/60 uppercase">
        {label}
      </p>
      <pre
        className={[
          "max-h-48 overflow-auto rounded-xl border p-3.5 font-mono text-xs whitespace-pre-wrap",
          tone === "fail"
            ? "border-rose-400/20 bg-rose-950/25 text-rose-200/90"
            : "border-white/10 bg-black/30 text-violet-100/85",
        ].join(" ")}
      >
        {body}
      </pre>
...
function Diff({
  ...
  return (
    <div>
      <p className="mb-1 text-[10px] font-semibold tracking-wider text-violet-300/50 uppercase">
        {label}
      </p>
      <pre
        className={[
          "overflow-x-auto rounded-lg border p-2.5 font-mono text-xs whitespace-pre-wrap",
          ok
            ? "border-emerald-400/20 bg-emerald-950/20 text-emerald-100/90"
            : "border-rose-400/20 bg-rose-950/20 text-rose-100/90",
        ].join(" ")}
      >
        {body || "(empty)"}
      </pre>
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/4 p-3">
      <p className="text-[10px] font-semibold tracking-wider text-violet-300/55 uppercase">
        {label}
      </p>
      <p
        className={[
