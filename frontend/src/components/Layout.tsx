...
export function Layout() {
  ...
  return (
    <div className="relative flex min-h-dvh flex-col">
      ...
      <header className="sticky top-0 z-40 border-b border-white/8 bg-[#07060f]/60 backdrop-blur-xl">
        <nav className="mx-auto flex h-16 max-w-7xl items-center gap-6 px-4 sm:px-6">
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            ...
            <span className="hidden text-[15px] font-bold tracking-tight text-violet-50 sm:block">
              Crucible
            </span>
          </Link>
          ...
          <div className="ml-auto flex items-center gap-2">
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <span
                  className="grid size-9 place-items-center rounded-full bg-gradient-to-br from-violet-500/80 to-fuchsia-500/80 text-sm font-bold text-white"
                  aria-hidden="true"
                >
      ...
      <footer className="border-t border-white/8 py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 text-sm text-violet-200/45 sm:flex-row sm:px-6">
          <p>
            Crucible — sandboxed competitive-programming judge by{" "}
            <a
...
/**
 * The API sleeps on Render's free tier and takes ~50s to wake. Rather than
 * letting the first page look broken, probe on boot and explain the wait.
 */
function ColdStartNotice() {
  ...
  useEffect(() => {
    ...
    // Only surface the banner if the wake-up is actually slow; a warm API
    // answers in well under a second and the user should never see this.
