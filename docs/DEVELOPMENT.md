...
| **B — Docker Compose** | Postgres container | **`docker`** | You need the real container sandbox, or all three languages |
| **C — Local Postgres** | Your own server | `local` or `docker` | You want Postgres without Compose |
...
Confirm the container sandbox is active:
...
│   ├── services/                 leaderboard.py · stats.py
...
├── docker-compose.yml            Postgres + API with the real sandbox
