...
Two configuration details make this work:

```yaml
volumes:
  - /var/run/docker.sock:/var/run/docker.sock   # daemon access
  - /tmp/judge:/tmp/judge                        # SAME path both sides
environment:
  EXECUTION_WORKDIR: /tmp/judge
```

Bind mounts are resolved by the **daemon on the host**, so a path that exists
only inside the API container mounts as empty. Mounting the same absolute path
on both sides makes it valid for both.

> ⚠️ Mounting the Docker socket grants **root-equivalent access to the host**.
> For anything internet-facing, run the judge on a separate worker host — see
> [Security](SECURITY.md#deployment-hardening).
