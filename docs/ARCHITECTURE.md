...
How Crucible is put together, and why each piece is shaped the way it is.
...
| **Services** | `app/services/` | Logic too involved for a handler | `leaderboard`, `stats` — pure functions over a session |
...
sequenceDiagram
    ...
    Note over A,DB: Persisted BEFORE judging, so a crash leaves<br/>an auditable row rather than a lost attempt
...
<details>
<summary><b>Why judge synchronously instead of using a queue?</b></summary>

Judging is bounded: `test_count × time_limit`, and evaluation stops at the first
failure. For these problem sizes that is well under a second in the common case.
A synchronous response means the client gets a verdict without polling, which is
a materially better API.
