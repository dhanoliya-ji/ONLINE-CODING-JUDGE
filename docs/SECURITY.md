...
3. **Never mount the Docker socket into an internet-facing API.** `docker-compose.yml`
   does, for local development, and says so — access to that socket is
   root-equivalent on the host.
4. **Narrow CORS.** `CORS_ORIGINS=*` is the default for demo convenience. Set it
   to your frontend's origin.
5. **Add rate limiting.** A reverse-proxy limit or `slowapi` in front of
   `POST /submissions`.
6. **Restrict the database role.** The application needs DML, not `SUPERUSER`.
7. **Close registration** or add email verification if the judge is not meant to
   be public.
8. **Rotate `SECRET_KEY`** on any suspected compromise; it invalidates every
   issued token.
