...
Submit in Python, C++ or Java and get a verdict in the same HTTP response — graded against
sample and hidden test suites inside an isolated sandbox with hard time, memory, CPU and
process limits.
...
- 88 automated tests, GitHub Actions CI
...
4. **`create_all()` is not a migration system.** New tables appear; changed
   columns do not.
5. **No rate limiting.** A reverse-proxy limit or `slowapi` is the straightforward addition.
...
- [ ] **Celery + Redis queue** — asynchronous judging with a status poll
