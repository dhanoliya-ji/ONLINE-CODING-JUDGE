...
from app.execution.judge import normalise_output
...
def submit(client, headers, problem_id, source, language="python", contest_id=None):
    payload = {
    ...
    return client.post(f"{API}/submissions", headers=headers, json=payload)
...
def test_hidden_test_data_never_leaves_the_server(client, user_headers, problem_id):
    ...
    assert "30" not in str(body["test_results"])


def test_sample_results_include_a_diff(client, user_headers, problem_id):
    body = submit(client, user_headers, problem_id, SOLUTION_WRONG).json()
    sample = next(r for r in body["test_results"] if r["is_sample"])
